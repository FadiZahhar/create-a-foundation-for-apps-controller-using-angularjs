(function() {
  ...
  function TasksController($scope, $controller) {
    ...
    $scope.CreateTask = function(title, body, tags){
      ...
      // set the default task color class to a blank string
      task.color = '';

      // set initial task status to "incomplete"
    ...
    $scope.SetTaskColor = function(id, color){
      // filter through $scope.tasks
      var findTask = $scope.tasks.filter(function(element) {
        // when we find the task with the id we're trying to target
        if (element.id == id) {
          // ...change its color value to the color that was passed through this function
          element.color = color;
          // then return it back into the array
          return element;

        } else {
          // pass all other tasks back into the array unchanged
          return element;
    ...
    $scope.SetTaskStatus = function(id, status){
      // filter through $scope.tasks
      var findTask = $scope.tasks.filter(function(element) {
        // ...until we find the task with the passed id value
        if (element.id == id) {
          // then update the status of that task
          element.status = status;
          // and return it into the array
          return element;

        } else {
          // pass other tasks back in unchanged
          return element;
